...
const Header = () => {
    ...
    const items =   (
        (!isAuth&&[{name: 'Вызвать мастера',path: '/'},{name: 'Войти',path: '/auth'}]) || 
        (isAuth&&[{name: 'Вызвать мастера',path: '/'},{name: `${currentUserName}`,path: '/admin'}])
    )
    ...
    return ( 
        <div className={'header'}>
            ...
            <div className='linksWrapper'>
                <NavBar items = {items} />
                {isAuth&&<div className='link' onClick = {()=>logout()} >Выйти</div>}
            </div>
        </div>
     );
}
