...
import { IFeedback } from '../../../types/order.types'

interface IProps {
  onSubmit: (formData: IFeedback) => void
  goBack: (e: React.SyntheticEvent<EventTarget, Event>) => void
}
...
const AddFeedbackForm: FC<IProps> = ({ onSubmit, goBack }) => {
  const {
    control,
    handleSubmit,
    formState: { errors }
        ...
        <Controller
          ...
          render={({ field: { value, onChange } }) => (
            <Textarea
              ...
              endDecorator={
                <Typography variant='body1' sx={{ ml: 'auto', fontSize: '14px' }}>
                  {value.length}/256
                </Typography>
              }
            />
