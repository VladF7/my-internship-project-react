...
import { IStatusOption } from '../../../types/status.types'
import { IOrdersFilterFormFields, IOrdersFilterForm } from '../../../types/order.types'
import { IOption } from '../../../types'

interface IProps {
  onSubmit: (formData: IOrdersFilterForm) => void
}
...
const OrdersFiltersForm: FC<IProps> = ({ onSubmit }) => {
  const { control, handleSubmit, reset } = useForm<IOrdersFilterFormFields>({
    mode: 'onBlur',
  ...
  const [statusesOptions, setStatusesOptions] = useState<IStatusOption[]>([])
  const [minMaxDateOptions, setMinMaxDateOptions] = useState<Date[]>([])
  const [minMaxPriceOptions, setMinMaxPriceOptions] = useState<number[]>([])
  ...
  const submit = (formData: IOrdersFilterFormFields) => {
    formData.minMaxPrice = formData.minMaxPrice.map((price) => formatValueToInteger(price))

    setDisableApplyFiltersButton(true)

    onSubmit({ ...formData, masters: formData.masters.map((master) => master.value as number) })
